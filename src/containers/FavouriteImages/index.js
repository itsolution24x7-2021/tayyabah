...
const FavouriteImageScreen = (props) => {
  ...
  const getFavouritePhotos = async () => {
    const {uid} = loginResponse.data.data;
    setIsLoading(true);

    let images = await getResourceFromSubCollection({
      collectionName: 'user',
      collectionDocId: uid,
  ...
  useEffect(() => {
    if (images.length > 0)
      setImageUrl(images[activeIndex]?._data?.imagesInfo?.image);
    ...
    if (isConnected) {
      RNFetchBlob.config({fileCache: true})
        ...
        .then(async (base64Res) => {
          console.log(
            '🚀 ~ file: index.js ~ line 168 ~ .then ~ base64Res',
            base64Res,
          );
  ...
  const addToDisLike = async () => {
    ...
    try {
      let result = await deleteResourceFromNestedSubCollection({
        collectionName: 'photoCategory',
        collectionDocId: images[activeIndex]?._data.imagesCategoryId,
        subCollectionName: 'photos',
        subCollectionDocId: images[activeIndex]?._data.imagesId,
        innerCollectionName: 'likes',
        innerCollectionDocId: images[activeIndex]?._data.likesId,
      });
      ...
      await deleteResourceNestedCollection({
        ...
        collectionDocId: uid,
        subCollectionName: 'imageLikes',
        subCollectionDocId: images[activeIndex]?._data.likesId,
      });
    ...
    if (commentMsg) {
      if (isConnected) {
        ...
        const payload = {
          ...
          msg: commentMsg,
          timeStamp: Date.now(),
          categoryId: images[activeIndex]._data.imagesCategoryId,
          photoId: images[activeIndex]._data.imagesId,
          commentType: 'photo',
        };
        try {
          setIsLoading(true);
        ...
        <View style={styles.secHeaderContainer}>
          {images.length > 0 && (
            <TouchableOpacity
              ...
              onPress={!isLike ? addToDisLike : null}>
              {isLike ? (
                <Image
...
export default FavouriteImageScreen;
